...
def test_model_registry_structure():
    ...
    assert "claude-opus-4-7" in registry.models
    assert "claude-sonnet-4-6" in registry.models
    assert "grok-4.20-reasoning" in registry.models
    ...
    grok = registry.models["grok-4.20-reasoning"]
    assert grok["provider"] == "openai_compat"
    assert grok["model_id"] == "grok-4.20-0309-reasoning"
...
def test_get_model_config():
    ...
    config = get_model_config("claude-opus-4-7")
...
def test_validate_model_availability():
    ...
    with patch.dict("os.environ", {
        ...
        assert validate_model_availability("claude-opus-4-7") is True
        assert validate_model_availability("grok-4.20-reasoning") is True
        assert validate_model_availability("glm-5") is True
        assert validate_model_availability("kimi-k2.6") is True

    # Test with missing API key
    with patch.dict("os.environ", {}, clear=True):
        assert validate_model_availability("claude-opus-4-7") is False
...
def test_model_role_mapping():
    ...
    # Portfolio Manager should use Opus 4.7
    pm_model = registry.get_model_for_role("portfolio_manager")
    assert pm_model == "claude-opus-4-7"
